-- Parent table: one row per transaction (income OR expense).
-- Holds the shared identity/metadata of an entry.
...
-- Child table: amounts for income entries. 1:1 with an entry of type 'income'.
...
-- Child table: amounts for expense entries. 1:1 with an entry of type 'expense'.
