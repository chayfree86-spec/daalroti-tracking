...
export const CategoryMergeModal = ({ 
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 dark:bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 md:p-6 animate-in fade-in duration-200">
      <div 
        className="bg-slate-50 dark:bg-slate-900 w-full max-w-4xl rounded-[2.5rem] shadow-2xl border border-white/60 dark:border-slate-800 overflow-hidden flex flex-col max-h-[92vh] transition-colors"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Header */}
        <div className="p-5 sm:p-7 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between gap-4 shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-2xl bg-amber-500/10 dark:bg-amber-500/20 text-amber-600 dark:text-amber-400 flex items-center justify-center shadow-xs shrink-0">
              <Layers size={22} />
            </div>
            <div>
              <h2 className="text-xl sm:text-2xl font-black text-slate-800 dark:text-white tracking-tight">Merge & Split Categories</h2>
          ...
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 flex items-center justify-center transition-all cursor-pointer active:scale-95 shrink-0"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="p-4 sm:p-6 lg:p-8 overflow-y-auto space-y-6 flex-1 no-scrollbar">
          ...
          {smartSuggestions.length > 0 && (
            <div className="p-4 sm:p-5 rounded-[2rem] bg-gradient-to-br from-amber-500/10 via-amber-500/5 to-transparent dark:from-amber-500/15 dark:via-amber-500/5 border border-amber-300/60 dark:border-amber-700/60 space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 text-amber-950 dark:text-amber-200 font-black text-sm">
                ...
                <span className="text-[10px] bg-amber-200/80 dark:bg-amber-900/60 text-amber-900 dark:text-amber-300 px-2.5 py-0.5 rounded-full font-bold self-start sm:self-auto">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
                {smartSuggestions.map((sug, idx) => (
                  <div 
                    key={idx}
                    className="p-3 sm:p-3.5 rounded-2xl bg-white dark:bg-slate-800 border border-amber-200/80 dark:border-amber-800/60 shadow-xs flex items-center justify-between gap-2.5"
                  >
                    <div className="min-w-0 space-y-1 flex-1">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        {sug.categories.map((c, i) => (
                          <span key={i} className="px-2 py-0.5 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 text-[11px] font-black border border-slate-200 dark:border-slate-600 truncate max-w-[130px] sm:max-w-[150px]">
                            {c}
                          </span>
                        ))}
                      </div>
                      <p className="text-[10px] font-bold text-amber-700 dark:text-amber-400 flex items-center gap-1 truncate">
                        <ArrowRight size={10} className="shrink-0" /> Merge into: <span className="font-black text-slate-800 dark:text-white truncate">{sug.suggestedName}</span>
                      </p>
                    </div>
          ...
          {activeGroups.length > 0 && (
            <div className="bg-white dark:bg-slate-800/70 p-5 sm:p-6 rounded-[2rem] shadow-premium border border-slate-100 dark:border-slate-800 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {activeGroups.map((grp) => (
                  <div 
                    key={grp.primaryName}
                    className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700 flex items-center justify-between gap-3"
                  >
                    <div className="min-w-0 space-y-1.5">
                      <div className="flex items-center gap-2">
                        <span className="font-black text-slate-800 dark:text-white text-sm">{grp.primaryName}</span>
                      ...
                      <div className="flex items-center gap-1 flex-wrap">
                        {grp.items.map((item, i) => (
                          <span key={i} className="text-[10px] font-bold text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-900 px-2 py-0.5 rounded-md border border-slate-200 dark:border-slate-700">
                            {item}
                          </span>
                        ))}
                      </div>
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-xl bg-rose-50 dark:bg-rose-950/50 hover:bg-rose-100 dark:hover:bg-rose-900/50 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800 font-black text-xs uppercase tracking-wider transition-all cursor-pointer active:scale-95 shrink-0 flex items-center gap-1"
          ...
          <div className="bg-white dark:bg-slate-800/70 p-5 sm:p-6 rounded-[2rem] shadow-premium border border-slate-100 dark:border-slate-800 space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100 dark:border-slate-800">
              ...
              <div className="relative w-full sm:w-64">
                ...
                <input 
                  ...
                  className="w-full pl-9 pr-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-700 dark:text-white placeholder:text-slate-400 focus:outline-none focus:border-primary/40"
                />
              </div>
            </div>
            ...
            {selectedItems.length > 0 && (
              <div className="p-3.5 rounded-2xl bg-primary/5 dark:bg-primary/10 border border-primary/20 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <span className="w-6 h-6 rounded-full bg-primary text-white text-xs font-black flex items-center justify-center">
                    {selectedItems.length}
                  </span>
                  <span className="text-xs font-black text-slate-800 dark:text-white">categories selected</span>
                </div>
                ...
                <div className="flex items-center gap-2 flex-1 max-w-md">
                  <input
                    ...
                    className="flex-1 px-3 py-1.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-800 dark:text-white focus:outline-none focus:border-primary"
                  />
                  <button
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isMergedGroup ? "bg-amber-50/70 dark:bg-amber-950/40 border-amber-300/80 dark:border-amber-700/80 text-amber-950 dark:text-amber-200 hover:bg-amber-100/60 dark:hover:bg-amber-900/60" :
                      "bg-white dark:bg-slate-800 border-slate-200/80 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:border-slate-300 dark:hover:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700/60"
                    )}
                  >
                    ...
                    <div className={cn(
                      ...
                      isSelected ? "bg-white text-primary border-white" : "border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900"
                    )}>
        ...
        {/* Modal Footer */}
        <div className="p-4 sm:p-5 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end shrink-0">
          <button
            onClick={onClose}
            className="px-6 py-2.5 rounded-xl bg-slate-900 dark:bg-slate-800 text-white font-black text-xs uppercase tracking-wider hover:bg-slate-800 dark:hover:bg-slate-700 transition-all cursor-pointer active:scale-95 shadow-md"
          >
            Done
          </button>
        </div>
